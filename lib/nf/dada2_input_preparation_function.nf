...
def dada2_input_preparation_function( concat_input, reads_qc, cutadapt_channel ) {
    ...
    def dada2_input = concat_input
                  .map{ meta, std_primer, auto_primer ->                                                                      // 1.
                    def key = groupKey(meta.subMap('id', 'single_end'), meta['var_regions_size'])                               // 1.
                  ...
                  .map{ meta, var_region, var_regions_size, reads ->                                                            // 3.
                    def final_var_region = var_region.unique()[0]                                                               // 3.A
                    def final_var_region_size = var_regions_size[0][0]                                                          // 3.A
                    def final_meta = meta + ['var_region': final_var_region, 'var_regions_size': final_var_region_size]         // 3.A

                    def fastp_reads = reads[0]                                                                                  // 3.B
                    def cutadapt_reads = reads[1]                                                                               // 3.B
                                                                                                                                // 3.B
                    def cutadapt_read_size = 0                                                                                  // 3.B
